import recryptJsLib from 'recryptjs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'recrypt-opcodes'
...
export default class Ledger {
  ...
  static async connect() {
    const recrypt = new btcApp(await transportU2f.create())
    // ensure in Recrypt App
    const pubkeyRes = await recrypt.getWalletPublicKey(Ledger.defaultPath)
    if (pubkeyRes.bitcoinAddress[0] !== 'Q') {
      throw 'Not Recrypt App'
    }
    return new Ledger(recrypt)
  }
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const pubkeyRes = await ledger.recrypt.getWalletPublicKey(path)
    ...
    const inputs = []
    const paths = []
    const selectUtxo = recryptJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    for(let i = 0; i < selectUtxo.length; i++) {
      ...
      inputs.push([
        await ledger.recrypt.splitTransaction(rawTxCache[item.hash]),
        item.pos
      ])
    }
    const outputs = new recryptJsLib.TransactionBuilder(keyPair.network)
    ...
    return await ledger.recrypt.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
  }
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new recryptJsLib.TransactionBuilder(keyPair.network)
    const contract =  recryptJsLib.script.compile([
      OPS.OP_4,
